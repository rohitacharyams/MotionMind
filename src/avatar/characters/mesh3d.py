...
# CesiumMan-specific joint name mapping (exact match on cleaned name)
_CESIUMMAN_MAP = {
    'torso_joint_1':    'Hips',
    'torso_joint_2':    'Spine2',
    'torso_joint_3':    'Neck',
    'neck_joint_1':     'Neck',
    'neck_joint_2':     'Head',
    'arm_joint_l__4_':  'LeftArm',
    'arm_joint_l__3_':  'LeftForeArm',
    'arm_joint_l__2_':  'LeftHand',
    'arm_joint_r':      'RightArm',
    'arm_joint_r__2_':  'RightForeArm',
    'arm_joint_r__3_':  'RightHand',
...
def _match_joint_name(gltf_name):
    ...
    # Try CesiumMan exact map first
    if clean in _CESIUMMAN_MAP:
        return _CESIUMMAN_MAP[clean]
    ...
    # Substring match (only long keywords to avoid false positives)
    for std_name, keywords in _JOINT_KEYWORDS.items():
        for kw in keywords:
...
class _GLTFSkin:
    ...
    def __init__(self, glb_path):
        ...
        self._skin_world_transforms = {}  # joint world transforms in skin/mesh space
    ...
    def _parse(self, path):
        ...
        # Compute skin-space world transforms (strip scene root transforms)
        # Must be called AFTER inv_bind_mats are loaded
        self._compute_skin_space_transforms()
    ...
    def _compute_skin_space_transforms(self):
        """Compute joint world transforms in skin/mesh local space.

        Scene graphs often have root transforms (Z_UP rotations, armature nodes)
        above the skeleton. The IBMs are defined in skin space (without these
        root transforms). We detect and strip them so that:
            skin_world_j @ IBM_j ≈ identity for T-pose.
        """
        if not self.joints or self.inv_bind_mats is None:
            self._skin_world_transforms = dict(self.joint_world_transforms)
            return

        # Find the best inv_scene_root by testing: (inv_scene @ joint_world) @ IBM ≈ I
        # Walk up from the first skin joint to scene root, accumulating transforms
        root_ji = self.joints[0]
        # Build parent map
        parent_map = {}
        for ni, children in self.joint_children.items():
            for c in children:
                parent_map[c] = ni

        # Collect ancestor chain above the skeleton root
        ancestors = []
        current = parent_map.get(root_ji)
        while current is not None:
            ancestors.append(current)
            current = parent_map.get(current)
        ancestors.reverse()  # root first

        # Compute the scene root transform (product of all ancestors)
        scene_root = np.eye(4)
        for anc in ancestors:
            scene_root = scene_root @ self.joint_local_transforms.get(anc, np.eye(4))

        # Verify: (inv(scene_root) @ joint_world[0]) @ IBM[0] should be ~identity
        inv_scene = np.linalg.inv(scene_root)
        test = (inv_scene @ self.joint_world_transforms.get(root_ji, np.eye(4))) @ self.inv_bind_mats[0]
        if np.abs(test - np.eye(4)).max() < 0.01:
            logger.info("Stripped scene root transform (%d ancestor nodes)", len(ancestors))
            for ni, wt in self.joint_world_transforms.items():
                self._skin_world_transforms[ni] = inv_scene @ wt
        else:
            # No stripping needed
            self._skin_world_transforms = dict(self.joint_world_transforms)
    ...
    def get_tpose_joint_positions(self):
        """Extract T-pose joint positions in skin space."""
        positions = {}
        for idx, ji in enumerate(self.joints):
            swt = self._skin_world_transforms.get(ji)
            if swt is not None:
                pos = swt[:3, 3]
    ...
    def _compute_canon_to_model(self):
        """Compute rotation matrix from canonical Y-up to model skin space.

        Canonical: Y-up, arms along X.
        CesiumMan: Z-up, arms along Y.
        """
        # Get model joint positions in skin space
        model_pos = {}
        for idx, ji in enumerate(self.joints):
        ...
        # Compute model up axis (spine direction)
        if 'Hips' in model_pos and 'Head' in model_pos:
            up = model_pos['Head'] - model_pos['Hips']
        elif 'Hips' in model_pos and 'Neck' in model_pos:
            up = model_pos['Neck'] - model_pos['Hips']
        else:
            up = np.array([0.0, 0.0, 1.0])
        up = up / np.linalg.norm(up)

        # Compute model lateral axis (shoulder direction: left - right)
        if 'LeftArm' in model_pos and 'RightArm' in model_pos:
            left = model_pos['LeftArm'] - model_pos['RightArm']
        else:
            left = np.array([0.0, 1.0, 0.0])
        # Orthogonalize left against up
        left = left - np.dot(left, up) * up
        left = left / np.linalg.norm(left)

        # Forward = up x left (right-hand rule)
        fwd = np.cross(up, left)
        fwd = fwd / np.linalg.norm(fwd)

        # T maps canonical axes to model axes:
        # canonical X (right) -> model -left = -left
        # canonical Y (up)    -> model up
        # canonical Z (fwd)   -> model fwd
        # T columns = images of canonical basis vectors
        T = np.column_stack([-left, up, fwd])
        return T

    def compute_skinning_matrices(self, posed_joints):
        """Compute per-joint skinning matrices for LBS.

        Uses Forward Kinematics in model skin space: for each bone from
        root to leaf, compute the world-space rotation that swings the bone
        from its current direction to the desired direction, then apply
        that rotation (around the joint pivot) to the joint and all its
        descendants. This naturally handles hierarchical propagation.
        """
        n = len(self.joints)
        skinning_mats = np.zeros((n, 4, 4))

        # Coordinate transform: canonical Y-up → model skin space (cached)
        if not hasattr(self, '_canon_to_model'):
            self._canon_to_model = self._compute_canon_to_model()
        T = self._canon_to_model

        # Model T-pose positions for height scaling
        model_tpose = {}
        for idx in range(n):
            std = self._joint_to_std.get(idx)
            if std:
        ...
        # Scale factor: canonical height → model height
        ...
        model_head = model_tpose.get('Head', np.array([0, 0, 1.2]))
        canon_height = np.linalg.norm(canon_head - canon_hips)
        model_height = np.linalg.norm(model_head - model_hips)
        h_scale = model_height / canon_height if canon_height > 1e-6 else 1.0
        ...
        # Map std_name → joint list index
        std_to_idx = {}
        for idx_, std_ in self._joint_to_std.items():
            std_to_idx[std_] = idx_

        # Build parent map and descendants
        parent_map = {}
        for ni, children in self.joint_children.items():
            for c in children:
                parent_map[c] = ni
        ...
        def _get_descendants(idx):
            """Get all descendant joint list indices via BFS."""
            desc = []
            q = []
            ji = self.joints[idx]
            for c in self.joint_children.get(ji, []):
                if c in self.joints:
            ...
            while q:
                ...
                desc.append(ci)
                cji = self.joints[ci]
                for c in self.joint_children.get(cji, []):
                    if c in self.joints:
                        q.append(self.joints.index(c))
            return desc
        ...
        for idx in range(n):
            ji = self.joints[idx]
            posed_world[idx] = self._skin_world_transforms.get(
                ji, np.eye(4)
            ).copy()

        # BFS from root: apply FK rotations
        processed = set()
        queue = []
        for idx in range(n):
            ji = self.joints[idx]
            pni = parent_map.get(ji)
            ...
            if std_name and child_std:
                ...
                if (child_idx is not None and
                    ...
                    if cur_len > 1e-6 and des_len > 1e-6:
                        ...
                        R = _rotation_between(cur_dir, des_dir)

                        # Apply R around this joint's position to self
                        # and all descendants
        ...
        for idx in range(n):
            ...
            if self.inv_bind_mats is not None:
                skinning_mats[idx] = pw @ self.inv_bind_mats[idx]
            else:
                ji = self.joints[idx]
                tpose_w = self._skin_world_transforms.get(ji, np.eye(4))
                skinning_mats[idx] = pw @ np.linalg.inv(tpose_w)
...
class Mesh3DStyle:
    ...
    def _render_3d(self, canvas, keypoints, scores, min_score):
        ...
        try:
            ...
            # Camera — compute from actual mesh bounds
            all_verts = np.vstack([g.vertices for g in posed_geoms])
            mesh_min = all_verts.min(0)
            mesh_max = all_verts.max(0)
            mesh_center = (mesh_min + mesh_max) / 2
            mesh_height = mesh_max[1] - mesh_min[1]
            cam_dist = max(mesh_height * 1.5, self.camera_distance)
            look_y = mesh_center[1]
            ...
            if not hasattr(self, '_offscreen_r') or self._offscreen_r is None or self._offscreen_w != w or self._offscreen_h != h:
                if hasattr(self, '_offscreen_r') and self._offscreen_r is not None:
                    try:
                        self._offscreen_r.delete()
                    except Exception:
                        pass
                self._offscreen_r = pyrender.OffscreenRenderer(w, h)
                self._offscreen_w, self._offscreen_h = w, h
            color, _ = self._offscreen_r.render(scene)
    ...
    def _apply_skinned_pose(self, posed_joints):
        ...
        # Apply LBS to vertices (in mesh-local space, typically Z-up)
        ...
        if isinstance(self._mesh, trimesh.Scene):
            for name, geom in self._mesh.geometry.items():
                ...
                # Apply the scene graph transform (handles Z-up → Y-up etc.)
                try:
                    T, _ = self._mesh.graph[name]
                    g.apply_transform(T)
                except Exception:
                    pass
                return [g]
